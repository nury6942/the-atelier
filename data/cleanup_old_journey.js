...
// ⛔⛔⛔ DEPRECATED — 2026-07-21 · 절대 실행 금지 ⛔⛔⛔
// 이 스크립트는 "최근 1시간 내 추가된 항목만 유지하고 나머지 삭제" 로직입니다.
// 2026-07-21 리베이스로 10월 일정 98개를 새로 만들었는데,
// 추가 후 1시간이 지난 시점에 이걸 실행하면 ★그 98개가 전부 삭제됩니다★.
// 짝이었던 bulk_add_2026_journey.js 도 함께 폐기되었습니다.
// (기록 보존을 위해 삭제하지 않고 가드만 걸어둠)

(async function cleanupOldJourney() {
  if (!confirm(
    '⛔⛔ 매우 위험 — 이 스크립트는 폐기되었습니다 ⛔⛔\n\n' +
    '이 스크립트는 "최근 1시간 내 추가분만 남기고 전부 삭제"합니다.\n\n' +
    '2026-07-21 리베이스로 만든 10월 일정 98개가\n' +
    '추가된 지 1시간이 지났다면 ★전부 삭제됩니다★.\n\n' +
    '정말로 실행하시겠습니까? (권장: 아니오)'
  )) { console.log('⛔ 폐기된 스크립트 — 실행 취소됨'); return; }

  if (!confirm('마지막 확인: 10월 일정이 삭제될 수 있습니다.\n그래도 진행할까요?')) {
    console.log('⛔ 실행 취소됨'); return;
  }
