...
**1. Money 가계부 등록 안 되던 버그 픽스 ⭐**
- 원인: 입력 행 마지막 셀에 X(취소) 버튼만 있고 **저장 버튼이 아예 없었음**. Enter 키로만 저장 가능 → 마우스 사용자는 등록 불가
- 수정: `ldgInputRowHTML()` 마지막 셀에 인디고 ✓(저장, `ldgSaveInput()`) 버튼 + 회색 X(취소) 버튼 두 개 배치. 툴팁에 단축키 명시 (`저장 (Enter)`, `취소 (Esc)`)
