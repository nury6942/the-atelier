...
- 가로축: **올해 1~12월**(헤더에서 보는 연도 기준), 미래 달/데이터 없는 달은 선 끊김
- 실선+영역(인디고) = 올해 각 달 실측, **점선(슬레이트) = 작년 같은 달(YoY)** — 일별 차트의 '전월 같은요일' 점선과 동일 컨셉
- 곡선은 **부드러운 베지어(Catmull-Rom)**, null 구간 분리 처리. 호버 툴팁(달·올해·작년·증감%·진행 중) + vline. 범례(올해/작년) 추가
- 메타: `올해 누적 · 작년 동기간 · ±%`
- 결정(누리): 비교선=작년 같은 달, 기간=올해 1~12월
- 검증: `node --check` OK, 배선(renderForMonth/init/HTML ID) grep 확인

### 🎯 다음 할 일
- 라이브에서 위젯1 4열 레이아웃 + 카드 통계 시인성 눈으로 확인 (KRW 큰 값일 때 통계 셀 줄바꿈 여부)
- 월별 추이: 진행 중(이번 달) 부분합으로 끝점이 꺾여 보이는지 확인 — 거슬리면 끝점 점선/할로우 처리 고려
